...
pub fn get_optimal_params(
    ...
    for _ in 0..5 {
        let mut options = SAOptions {
            initial_step_size_ls: min_options.initial_step_size_ls * 0.1,
            ..Default::default()
        };
        let val = run_sa(problem, options).unwrap().value;
        if val < min_val {
            min_options = options;
            min_val = val;
        }
...
fn get_value_changes(
    ...
    for option in Option::iterator() {
        match option {
            ...
            AnnealingSchedule => {
                option_value_changes.push(get_anneačing_schedule_change(
                    base_value,
                    options.annealing_schedule,
                    problem,
                ))
            }
            //MaxIterationsSa => { option_value_changes.push(get_max_iter_sa_change(base_value, options.max_iterations_sa, problem)) }
            MaxIterationsLs => {
                option_value_changes.push(get_max_iter_ls_change(
                    base_value,
                    options.max_iterations_ls,
                    problem,
                ))
            }
            ...
            BestNSa => option_value_changes.push(get_best_n_sa_change(
                base_value,
                options.n_best_sa,
                problem,
            )),
            BestNLs => option_value_changes.push(get_best_n_ls_change(
                base_value,
                options.n_best_ls,
                problem,
            )),
...
fn generate_neighborhood(
    ...
    let mut res = Vec::new();
    for i in 0..2 {
        res.append(&mut generate_neighbors(
            options.clone(),
            value_changes[i].option.clone(),
        ))
    }
    ...
    match to_change {
        InitialTemperature => {
            for i in -5..5.to_i32().unwrap() {
                new_options.push(SAOptions {
                    initial_temperature: (options.initial_temperature as i32
                        + i * 2) as u8,
                    ..SAOptions::default()
                })
            }
        ...
        AnnealingSchedule => {
            for i in -4..15.to_i32().unwrap() {
                new_options.push(SAOptions {
                    annealing_schedule: options.annealing_schedule
                        + i as f64 * 0.01,
                    ..SAOptions::default()
                })
            }
        ...
        MaxIterationsSa => {
            for i in -10..10.to_i32().unwrap() {
                new_options.push(SAOptions {
                    max_iterations_sa: (options.max_iterations_sa as i32
                        + i * 1000)
                        as u64,
                    ..SAOptions::default()
                })
        ...
        MaxIterationsLs => {
            for i in -10..10.to_i32().unwrap() {
                new_options.push(SAOptions {
                    max_iterations_ls: (options.max_iterations_ls as i32
                        + i * 1000)
                        as u64,
                    ..SAOptions::default()
                })
        ...
        InitialStepSizeSa => {
            for i in -10..10.to_i32().unwrap() {
                new_options.push(SAOptions {
                    initial_step_size_sa: options.initial_step_size_sa
                        + i as f64 * 0.1,
                    ..SAOptions::default()
                })
            }
        ...
        InitialStepSizeLs => {
            for i in -10..10.to_i32().unwrap() {
                new_options.push(SAOptions {
                    initial_step_size_ls: options.initial_step_size_ls
                        + i as f64 * 0.1,
                    ..SAOptions::default()
                })
            }
        ...
        MinTemp => {
            for i in -5..5.to_i32().unwrap() {
                new_options.push(SAOptions {
                    min_temp: options.min_temp + i as f64,
                    ..SAOptions::default()
                })
            }
        }
        BestNSa => {
            for i in 0..8 {
                new_options.push(SAOptions {
                    initial_temperature: options.initial_temperature + i,
                    ..SAOptions::default()
                })
            }
        }
...
fn get_initial_step_change_sa(
    ...
    let mut res = run_sa(
        problem,
        SAOptions {
            initial_step_size_sa: initial_step + 0.1,
            ..SAOptions::default()
        },
    );
    let mut direction = Direction::Negative;
...
fn get_max_iter_ls_change(
    ...
    let mut res = run_sa(
        problem,
        SAOptions {
            max_iterations_ls: current_iter + 500,
            ..SAOptions::default()
        },
    );
    let mut direction = Direction::Negative;
...
fn get_max_iter_sa_change(
    ...
    let mut res = run_sa(
        problem,
        SAOptions {
            max_iterations_sa: current_iter + 500,
            ..SAOptions::default()
        },
    );
    let mut direction = Direction::Negative;
...
fn get_anneačing_schedule_change(
    base_value: f64,
    current_schedule: f64,
    problem: &mut BBOBProblem,
) -> OptionValue {
    let mut res = run_sa(
        problem,
        SAOptions {
            annealing_schedule: current_schedule + 0.02f64,
            ..SAOptions::default()
        },
    );
    let mut direction = Direction::Negative;
...
fn get_initial_temp_change(
    ...
    let mut res = run_sa(
        problem,
        SAOptions {
            initial_temperature: initial_temp + 5,
            ..SAOptions::default()
        },
    );
    let mut direction = Direction::Negative;
