...
impl SANeighborhood {
    ...
    pub fn generate_neighborhood(&mut self, current_state: State, problem: &mut BBOBProblem, options: SAOptions) -> () {
        let changes = self.find_biggest_change(current_state.clone().vector, problem, options.initial_step_size_sa);
        self.states = Vec::new();
    ...
    fn find_biggest_change(&mut self, current_neighborhood: Vec<f64>, problem: &mut BBOBProblem, step_size: f64) -> Vec<VectorElement> {
...
impl LocalSearchNeighborhood {
    ...
    pub fn generate_neighborhood(&mut self, current_state: State, problem: &mut BBOBProblem, options: SAOptions) -> () {
        let changes = self.find_biggest_change(current_state.clone().vector, problem, options.initial_step_size_ls);
        self.states = Vec::new();
